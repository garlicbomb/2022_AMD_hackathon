module boss ( //stage 5 boss
    ...
    wire [6:0] sprite_render_x;         // spirte render x축, 7bit , 0~127
    wire [5:0] sprite_render_y;         // spirte render y축, 6bit , 0~ 63 
    ...
    localparam [0:3][2:0][7:0] palette_colors =  { // Color table , 4(color) * 3 (R,G,B) * 8bit(0~255) // 4행 3열 data, 각 pixel 은 8bit 
       // R      G       B // 
        ...
        8'h8F, 8'h5E, 8'h00,    // R - 135 , G- 94 , B - 0 : Brown -> hexadecimal : 8F 00 00       ===> BROWN : 4'd1(main 뱅기 색)
        8'hFF, 8'hFF, 8'hFF,    // R - 255, G - 255, B - 255 : White -> hexadecimal : FF FF FF  ===> WHITE : 4'd2(죽어야 하는 뱅기 표시)
        8'h5D, 8'h5D, 8'h5D    // R - 93, G - 93, B - 93 : gray -> hexadecimal : 93 93 93     ===> GRAY : 4'd3(테두리)
    };
   
    localparam [0:56][0:100][3:0] sprite_data = {     // pixel data 16 X 16 ( data 상으로 15 행 16 열 ( 16 X 15 ) 각각의 color table 0~3 으로 총 네개 (black, red, white, blue)
    ...
    reg [5:0] notcollision = 50;  //50번 맞으면 없어짐
    ...
    /////////////////////////////////////////// modified ////////////////////////////////////////////////////////////////////////////////////
     initial begin
        sprite_x <= 16'd640 ; // 1280 가로의 중간인 640 에서 적들 5개 이미지의 1/2 값인 190을 뺌
        //-> 적들이 가운데에 위치할 수 있도록 
    ...
    always @(posedge i_v_sync) begin    // 적기가 총알에 부딪히는지 검사, notcollision signal을 발생시킴
        ...
        if(hit_sig==1) begin
            ...
            sprite_x <= sprite_x; 
                ...
                if ( notcollision == 0);  // 부딪히면 notcollision = 0이 됨 -> 전투기 표시되지 않음  
