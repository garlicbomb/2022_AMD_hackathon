module enemy_big_red_green_1( //보스친구(총이 나가지 않음.)
    ...
    localparam [0:3][2:0][7:0] palette_colors =  { // Color table , 4(color) * 3 (R,G,B) * 8bit(0~255) // 4행 3열 data, 각 pixel 은 8bit 
       // R      G       B // 
        ...
        8'h5F, 8'h0, 8'hFF,    // R - 95, G - 0, B - 255 : Violet -> hexadecimal : FF FF 21  ===> VIOLET : 4'd2(죽어야 하는 뱅기 표시)
    ...
    reg [2:0] notcollision = 5;  //7번 맞으면 없어짐
    ...
    /////////////////////////////////////////// modified ////////////////////////////////////////////////////////////////////////////////////
     initial begin
        sprite_x <= 16'd640 - 16'd30; // 1280 가로의 중간인 640 에서 적들 5개 이미지의 1/2 값인 190을 뺌
        //-> 적들이 가운데에 위치할 수 있도록 
    ...
    always @(posedge i_v_sync) begin    // 적기가 총알에 부딪히는지 검사, notcollision signal을 발생시킴
        ...
        shoot <= 0; end
        if(hit_sig==1) begin
            sprite_y <= sprite_y+3;        // 움직임을 위한 setting, y축에서 1의 속도로 내려옴
            sprite_x <= sprite_x; 
                ...
                if (sprite_y == 16'd144) begin
                    sprite_x <= sprite_x + (sprite_x_direction ? -5 : 5);
                    sprite_y <= sprite_y;
                    if (sprite_x == 16'd640 - 16'd30 + 16'd490)
                        sprite_x_direction <=1;
                    else if (sprite_x == 16'd640 - 16'd30 - 16'd490)
