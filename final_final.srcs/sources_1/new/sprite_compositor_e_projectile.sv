...
module sprite_compositor_e_projectile( // 총알 모듈 
    ...
    reg [15:0] sprite_x     = shoot_x + 8;   // sprite image x axis, 시작점
    reg [15:0] sprite_y     = shoot_y + 16;   // sprite image y axis, 시작점
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    wire [2:0] sprite_render_y;         // spirte render y축, 3bit , 0~ 7을 표현하기 위해  
     ...
     localparam [0:3][2:0][7:0] palette_colors =  { // Color table , 4(color) * 3 (R,G,B) * 8bit(0~255) // 4행 3열 data, 각 pixel 은 8bit 
       // R      G       B // 
        ...
        8'hF2, 8'h96, 8'h61     // R - 33, G - 33, B - 255 : ORANGE RED -> hexadecimal : 21 21 FF     ===> ORANGE RED : 4'd3
    };
   ...
   // 총알 
    ...
    // 가로는 총 4 * 2 = 8  임  
    // 세로는 총 14 * 4 = 56 
    4'd0,4'd0,4'd3,4'd3,4'd0,4'd0,
    4'd0,4'd3,4'd3,4'd3,4'd3,4'd0,
    4'd3,4'd3,4'd3,4'd3,4'd3,4'd3,
    4'd3,4'd3,4'd3,4'd3,4'd3,4'd3,
    4'd0,4'd3,4'd3,4'd3,4'd3,4'd0,
    4'd0,4'd0,4'd3,4'd3,4'd0,4'd0
    };
    ...
    always @(posedge i_v_sync) begin    // vertical sync -> 0, 1 clocking -> positive edge에서만 수행 
        ...
        if (enemy_shoot_signal == 1) begin
            ...
            if (sprite_y == shoot_y + 16) begin
                sprite_y = shoot_y+16+1; end
//            if ((sprite_y == shoot_y + 15) ||( sprite_y == shoot_y + 14 )) begin
//                sprite_y = shoot_y+16+1; end
            if ((sprite_y > shoot_y +16) && (sprite_y <= 720)) begin
                finish <= 0;
                sprite_x <= sprite_x;
                sprite_y <= sprite_y + 5; 
                end
                ...
                finish <= 1;
                sprite_x = shoot_x + 8;// sprite image x axis, 시작점
                sprite_y = shoot_y + 16;   // sprite image y axis, 시작점
        end
        end
        end
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
