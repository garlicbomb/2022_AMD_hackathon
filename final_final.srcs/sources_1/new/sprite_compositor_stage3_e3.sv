...
module sprite_compositor_stage3_e3(//왼쪽으로 가는 enemy
 ...
 //   reg shoot = 0;
  //  assign shoot_control = shoot;
    ...
    /////////////////////////////////////////// modified ////////////////////////////////////////////////////////////////////////////////////
     initial begin
        sprite_x <= 16'd1280; // x축 위에서 1280값을 간 상태
        //-> 적들이 가운데에 위치할 수 있도록 
        sprite_y <= 16'd360;           // y축은 위에서 360 값만큼 내려온 상태 
    ...
    always @(posedge i_v_sync) begin    // 적기가 총알에 부딪히는지 검사, notcollision signal을 발생시킴
     if(hit_sig==1) begin
       sprite_y <= sprite_y;        // 움직임을 위한 setting, x축에서 -1만큼 움직임
       sprite_x <= sprite_x-2; 
...
//      //  else if (sprite_x == 700 && notcollision == 2)       // shoot signal 
//        //    shoot <= notcollision - 1;                        // enemy가 살아있다면 총알을 발사하지만 그렇지 않다면 총알 발사 x -> notcollision = 0 : 충돌상태이므로 
//     //   else if (sprite_x == 700 && notcollision != 2)      
